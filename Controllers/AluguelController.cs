...
namespace LocadoraAPI.Controllers
{
    ...
    public class AluguelController : ControllerBase
    {
        ...
        public async Task<IActionResult> Cadastrar(Aluguel aluguel)
        {
            ...
            var cliente = await _context.Cliente.FirstOrDefaultAsync(x => x.Cpf == aluguel.Cli.Cpf);
            if (cliente == null)
            {
            ...
            var veiculo = await _context.Veiculo.FirstOrDefaultAsync(x => x.Placa == aluguel.VeiculoAlugado.Placa);
            if (veiculo == null)
            {
            ...
            aluguel.Cli = cliente;
            aluguel.VeiculoAlugado = veiculo;
        ...
        public async Task<ActionResult<IEnumerable<Aluguel>>> Listar()
        {
            var alugueisCompletos = await _context.Aluguel
                .Include(a => a.Cli)
                .Include(a => a.VeiculoAlugado)
                .ToListAsync();

            var resultado = alugueisCompletos.Select(aluguel => new
            {
                Id = aluguel.Id,
                Cli = aluguel.Cli?.Nome ?? "Cliente não encontrado",
                VeiculoAlugado = aluguel.VeiculoAlugado?.Placa ?? "Veículo não encontrado"
            });
        ...
        public async Task<IActionResult> ModificarVeiculoAlugado(string id, [FromForm] string veiculoAlugado)
        {
            ...
            aluguel.VeiculoAlugado = new Veiculo { Placa = veiculoAlugado };
